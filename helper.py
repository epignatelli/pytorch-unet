...
def save(folder, images, masks):
    os.makedirs(folder, exist_ok=True)

    for i in range(len(images)):
        im = Image.fromarray(images[i])
        mask = Image.fromarray(masks_to_colorimg(masks[i]))
        im.save(os.path.join(folder, "{}_image.jpg".format(i)))
        mask.save(os.path.join(folder, "{}_mask.jpg".format(i)))
